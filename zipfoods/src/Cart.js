...
export default class Cart {
    ...
    add(productId, quantity = 1) {
        ...
        if (item) {
            ...
            this.items[item.index]['quantity'] += quantity;
     ...
     */
    getItem(productId) {
        let item = this.items.find(({ id }) => id === productId) || null;

        // Add the index as a property of the item 
        // This is used when updating the item
        if (item) {
